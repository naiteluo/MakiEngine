#include "MemoryManager.hpp"

using namespace Me;

namespace Me {
    static const uint32_t kBlockSizes[] = {
            // 4-increments
            4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48,
            52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96,

            // 32-increments
            128, 160, 192, 224, 256, 288, 320, 352, 384,
            416, 448, 480, 512, 544, 576, 608, 640,

            // 64-increments
            704, 768, 832, 896, 960, 1024
    };

    // TODO: why 2^13?
    static const uint32_t kPageSize = 8192;
    // TODO: why alignment 4?
    static const uint32_t kAlignment = 4;

    // number of elements in the block size array
    static const uint32_t kNumBlockSizes =
            sizeof(kBlockSizes) / sizeof(kBlockSizes[0]);

    // largest valid block size
    static const uint32_t kMaxBlockSize =
            kBlockSizes[kNumBlockSizes - 1];

    size_t * MemoryManager::m_pBlockSizeLoopUp;
    Allocator* MemoryManager::m_pAllocators;
}

int Me::MemoryManager::Initialize() {
    // one-time initialization
    static bool s_bInitialized = false;
    if (!s_bInitialized) {
        // initialize block size lookup table
        m_pBlockSizeLoopUp = new size_t[kMaxBlockSize + 1];
        size_t j = 0;
        // 建立所有size和固定blockSize index的映射，减少实时通过size计算blockSize的运算消耗
        for (size_t i = 0; i < kMaxBlockSize; i++) {
            if (i > kBlockSizes[j]) ++j;
            m_pBlockSizeLoopUp[i] = j;
        }

        // initialize the allocators
        m_pAllocators = new Allocator[kNumBlockSizes];
        for (size_t i = 0; i < kNumBlockSizes; ++i) {
            m_pAllocators[i].Reset(kBlockSizes[i], kPageSize, kAlignment);
        }
        s_bInitialized = true;
    }
    return 0;
}

void Me::MemoryManager::Finalize() {
    delete[] m_pAllocators;
    delete[] m_pBlockSizeLoopUp;
}

void Me::MemoryManager::Tick() {

}

Me::Allocator *Me::MemoryManager::LoopUpAllocator(size_t size) {
    // check eligibility for lookup
    if (size <= kNumBlockSizes) {
        return m_pAllocators + m_pBlockSizeLoopUp[size];
    } else {
        return nullptr;
    }
}

void *Me::MemoryManager::Allocate(size_t size) {
    Allocator *pAlloc = LoopUpAllocator(size);
    if (pAlloc) {
        return pAlloc->Allocate();
    } else {
        return malloc(size);
    }
}

void Me::MemoryManager::Free(void *p, size_t size) {
    Allocator *pAlloc = LoopUpAllocator(size);
    if (pAlloc) {
        pAlloc->Free(p);
    } else {
        free(p);
    }
}
